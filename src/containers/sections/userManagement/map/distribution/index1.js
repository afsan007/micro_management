...
class MapDistribution extends Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      ...
      console.log('loading:', this.props.mapClicked);
      if (this.props.mapClicked.id !== 'world') {
        this.visitorSocket.emit('visitorsCountryDetail', {
          countryId: this.props.mapClicked.id,
          mapType: this.props.mapType,
        });
        this.setState({ worldAlarm: true, change_loading: true });
      } else {
        this.mapInitialData({ mapSpecificData: null, worldAlarm: false, change_loading: true });
      }
    }
    if (
      this.state.mapInitialData !== prevState.mapInitialData ||
  ...
  componentDidMount() {
    this.visitorSocket.on('visitorsCountryDetail_receive', reply => {
      console.log('data_Receive', reply);
      reply && reply !== '{}'
  ...
  mainData = () => {
    ...
    return mapData.map(data => (
      <div key={data.region} className={this.cn(['MapData-info'])}>
        <h5 className={this.cn(['mb-5'])} title={+data.count}>
          {shortNum(+data.count)}
        </h5>
        ...
        <div className={this.cn(['progress-bar'])}>
  ...
  backBtn = () => {
    return this.state.worldAlarm &&
      (Object.keys(this.state.mapInitialData).length === 0 || this.state.mapSpecificData) ? (
      <button
